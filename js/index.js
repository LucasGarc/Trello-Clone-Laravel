...
function makeBoardTitle(title, id) {
  return `<input type='text' value='${title}' class='inputEdit' id='input${id}' data-id='${id}' /> <span id='title${id}' class='boardTitle' data-id='${id}'>${title}</span> <i class='bi-pencil btnRight btnEditTitle' data-id='${id}'></i> <i class="bi bi-x-lg btnRemoveBoard btnCenter" data-id='${id}'></i> <br style='clear:both' />`;
}

function makeTaskTitle(title, id) {
  return `${title} <i class='bi bi-trash btnRight btnRemoveTask' data-id='${id}'></i>`;
}
...
function buildRemoveBoard() {
  let btnsRemoveBoard = document.getElementsByClassName(`btnRemoveBoard`);
  for (let index = 0; index < btnsRemoveBoard.length; index++) {
    const btnRemoveBoard = btnsRemoveBoard[index];

    btnRemoveBoard.addEventListener("click", function () {
      Swal.fire({
        title: 'Você tem certeza?',
        text: "Essa ação é irreversivel!",
        icon: 'warning',
...
var Kanban = new jKanban({
  ...
  buttonClick: function (el, boardRef) {
    ...
    formItem.innerHTML = '<div class="form-group"><textarea class="form-control" rows="2" autofocus></textarea></div><div class="form-group"><button type="submit" class="btn btn-primary btn-x5 pull-right">Salvar</button><button type="button" id="CancelBtn" class="btn btn-default btn-x5 pull-right">Cancelar</button></div>';
...
const getTasks = async () => {
  try {
    ...
    await axios.get('functions/index.php', { params: { type: "read_tasks" }, }, { headers: { 'Content-type': 'application/json' } }).then(function (response) {
        tasks = response.data;
...
const deleteBoard = async (boardId) => {
  ...
  try {
    ...
    await axios.get("functions/index.php", { params: { type:"delete_board", board_id: boardId }, }, { headers: { 'Content-Type': 'application/json' } })
      .then(function (response) {

        if (response.data.success == true) {

          Toast.fire({ icon: 'success', title: 'Board apagado com sucesso!' });
          Kanban.removeBoard(boardId);

        } else {
          Toast.fire({ icon: 'error', title: 'Aconteceu um erro ao atualizar o Board!' });
        }
      })
      .catch(function () {
        Toast.fire({ icon: 'error', title: 'Aconteceu um erro com a comunicação da base de dados!' });
      })
